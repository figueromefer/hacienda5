<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar cotización</h2>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="client_id" class="w-full border rounded">
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" @selected($quotation->client_id == $client->id)>
                                        {{ $client->full_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="event_id" class="w-full border rounded">
                                ...
                                @foreach($events as $event)
                                    <option value="{{ $event->id }}" @selected($quotation->event_id == $event->id)>
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full border rounded">
                                ...
                                <option value="sent" @selected($quotation->status === 'sent')>Enviada</option>
                                <option value="approved" @selected($quotation->status === 'approved')>Aprobada</option>
                                <option value="rejected" @selected($quotation->status === 'rejected')>Rechazada</option>
                                <option value="expired" @selected($quotation->status === 'expired')>Expirada</option>
                            </select>
                        </div>

                        <div>
                            <label class="block mb-1">Válida hasta</label>
                            <input type="date" name="valid_until" class="w-full border rounded" value="{{ old('valid_until', $quotation->valid_until?->format('Y-m-d')) }}">
                        </div>

                        <div>
                            <label class="block mb-1">Descuento</label>
                            <input type="number" step="0.01" min="0" name="discount" id="discount" class="w-full border rounded" value="{{ old('discount', $quotation->discount) }}">
                        </div>
                    </div>

                    <div>
                        <label class="block mb-1">Notas</label>
                        <textarea name="notes" class="w-full border rounded">{{ old('notes', $quotation->notes) }}</textarea>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            let index = {{ $quotation->items->count() }};
